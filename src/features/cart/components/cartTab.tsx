...
export function CartTab (){
    ...
    return (
            <div className={`${isCartModalOpen?'z-1':'w-[0px]'} absolute top-0  right-0 overflow-hidden`}>
                        <div className={`${isCartModalOpen? `translate-x-0 `: `translate-x-200`} cartTab_conatiner relative  bg-white w-[600px] h-[1300px] flex flex-col justify-between transition-transform duration-700 overflow-hidden`} >
                            <div>
                                <div className="mt-5 border-b border-[#dce3e5] text-[#737272] py-2 px-3 font-bold flex flex-row justify-between">
                                    <p>Shopping Cart</p>
                                    <span className="text-xl cursor-pointer " onClick={closeCartModal}>X</span>
                                </div>
                                <div className={`${cart?"":"hidden"} text-black px-3 pt-3`}>
                                    {cart?.map((product,i) => {
                                    return(
                                        <div key={i} className="flex flex-row relative my-5">
                                            <Image src={`${product?.imgThumb}`}  alt="cartImg" width={70} height={20}/>
                                            <div className="">
                                                <a href={`http://localhost:3000/products/${product?.productId}/${product?.slug}`}>
                                                    <p className="text-wrap text-sm text-[#818283] w-[400px] pl-5">{product?.name}</p> 
                                                </a>
                                                <div className="ml-5">
                                                    ...
                                                    <button  className='text-[#d2dcdf] text-base place-self-end 
                                                    cursor-pointer  px-1  border absolute top-0 right-0
                                                    rounded-full bg-white hover:text-white hover:bg-[#ff6b6b] transition-color duration-300' onClick={() => handleDeleteItem(product?.productId??"")}>X</button>
