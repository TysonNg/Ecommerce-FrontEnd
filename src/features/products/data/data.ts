...
export const  getProductsHotDeals = async() => {
    const res = await fetch(`${rootApi}/product/hotDeals`,{
        method: "GET",
        headers:{
          "x-api-key": apiKey 
        },
...
export const getProductDetail=  async(productId : string) => {
  ...
  const res = await fetch(`${rootApi}/product/${productId}`,{
    method: "GET",
    headers:{
      "x-api-key": apiKey 
    },
...
export async function getRelatedProductByCategory(category: string, productId: string, limit = 6 ) {
  const res = await fetch(`${rootApi}/product/categories?category=${category}&&limit=${limit}`,{
    method: "GET",
    headers:{
      "x-api-key": apiKey 
    },
...
export async function getAllProducts(limit:number,category: string, apiKey: string, page: string) {
  ...
  const res = await fetch(`${rootApi}/product${category?`/categories?category=${category}&&`:"?"}limit=${limit}&&page=${page}`,{
    method: "GET",
    headers:{
      "x-api-key": apiKey
    },
