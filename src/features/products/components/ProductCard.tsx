...
export function ProductGrid(props: Products) {
  ...
  return (
    <div className={`${styles.productCard_container} `}>
      <ul
        className={`${styles.prroductCart_list} grid ${
          numOfProduct === 4
            ? "xl:grid-cols-16 sm:grid-cols-8"
            : "xl:grid-cols-12 sm:grid-cols-8"
        } justify-items-center items-center mb-20`}
      >
...
export function ProductCard(props: ProductCardProps) {
  ...
  if (product_prevPrice) {
    return (
      <li
        key={_id}
        className="col-span-4 gap-2 p-4 bg-white rounded-lg text-dark place-items-center"
      ...
      >
        <div className={`${styles.thumbnail} relative`}>
          <a href={`/products/${_id}/${product_slug}`}>
            <span
              className={`${styles.saleTag} absolute top-3 left-3 border border-[#dce3e5] bg-white rounded-2xl text-xs text-[#48515b] p-1.5`}
            >
              Sale!
            </span>
            <Image
          ...
          <FontAwesomeIcon
            ...
            }  transition-opacity duration-300 hover:opacity-80 text-[#48515b] text-sm cursor-pointer absolute rounded rounded-full py-1.5 px-1.5 top-3 bg-[#f1e0e0]`}
        ...
        <div>
          <a
            ...
            className={`${styles.productName} text-sm font-semibold`}
          >
            {product_name}
          </a>
          <p>
            <span className="text-xs line-through text-[#48515b]">
