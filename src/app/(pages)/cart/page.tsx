...
export default function CartPage() {
    ...
    if (cart.length > 0) {
      setCheckout((prev) => {
        const updatedShopOrders = cart.map((item) => {
          const prevShop = prev.shop_order_ids.find(
        ...
        if (cart.length > 0) {
          if (checkout.shop_order_ids.length > 0) {
            const res = await checkoutReview(
              checkout?.shop_order_ids.length > 0
                ? checkout
                : {
                    userId: id ?? `${guestId}`,
    ...
    setCheckout((prevCheckout: CheckoutRequest) => {
      ...
      const updateCheckout = {
        ...
        shop_order_ids: prevCheckout.shop_order_ids.map((order: ShopOrder) => ({
          ...order,
          item_products: order.item_products.map(
            (product): Product =>
  ...
  const addDiscountToProduct = async (
    productId: string,
    price: number,
    quantity: number,
    codeId: string,
    ...
    if (codeId !== codeIdState?.codeId) {
      await cancelDiscount({
        codeId: codeIdState?.codeId ?? "",
        userId: id ?? "",
        shopId: codeIdState?.shopId ?? "",
      });
    }
    ...
    setCheckout((prevCheckout: CheckoutRequest) => {
      const updateCheckout: CheckoutRequest = {
        ...prevCheckout,
        shop_order_ids: prevCheckout.shop_order_ids.map((order: ShopOrder) =>
          order.item_products[0].productId === productId
            ? { ...order, shop_discount: [newDiscount] }
            : { ...order }
        ),
      };
      ...
      sessionStorage.setItem(
        `reviewCheckout_${cartUserId}`,
        JSON.stringify(updateCheckout)
      );
      return updateCheckout;
    });
  };

  const removeDiscountFromProduct = async ({
    productId,
    codeId,
    shopId,
  ...
  if (cart.length > 0) {
    return (
      <section>
        <div className="w-full max-w-[500px] md:max-w-[700px] xl:max-w-[1000px] 2xl:max-w-[1200px] mx-auto my-0 mt-[3rem] pb-50">
          <h1 className="font-bold text-center text-xl sm:text-2xl mb-[1.2rem]">Cart</h1>
          <div className="flex flex-col justify-between gap-5 place-items-center sm:flex-row sm:place-items-start ">
            <div className="cart-products border-1 border-[#9b9b9b] w-full max-w-[350px] sm:w-2/3 sm:max-w-full h-full">
              <div className="hidden sm:grid grid-cols-12 text-center text-[#696969] text-sm font-bold  py-2">
                <h3 className="col-span-1"></h3>
                <h3 className="sm:col-span-2 md:col-span-6">Product</h3>
                <h3 className="sm:col-span-2 md:col-span-1">Price</h3>
                <h3 className="sm:col-span-4 md:col-span-2">Quantity</h3>
                <h3 className=" sm:col-span-3 md:col-span-2">Subtotal</h3>
              </div>
              <div className="sm:hidden text-center text-[#696969] text-sm font-bold  py-2">
                Items
              </div>
              {cart?.map((product, i) => {
                return (
                  <div key={i}>
                    <div className="flex flex-col justify-items-center sm:flex-row content-center grid-cols-12 items-center gap-2 border-t border-[#9b9b9b] p-4 w-full">
                      <button
                        className="text-[#d2dcdf] text-base place-self-end col-span-1 
                                            cursor-pointer  px-1  border
                                            rounded-full bg-white hover:opacity-80 self-center"
                        onClick={() =>
                      ...
                      <div className="relative aspect-w-1 aspect-h-1 w-[100px] h-[100px]">
                        <Image
                      ...
                      <a
                        ...
                        className="col-span-4 w-full max-w-[150px] md:max-w-[300px] text-sm text-[#0573f0] truncate"
                      >
                        {product?.name}
                      </a>
                      <p className="col-span-2 text-[#696969]">
                        ${product?.price}.00
                      </p>
                    ...
                    <div
                      className="voucher-container w-full px-5 py-2 text-white flex flex-row justify-between cursor-pointer
                                         border-t border-[#9b9b9b] bg-[#d15454] hover:bg-[#b34747] transition-colors duration-500 "
                      onClick={() =>
                        getDiscountsOfProduct(product?.productId ?? "")
                      }
                    >
                      <div>
                        <FontAwesomeIcon
            ...
            <div className="cart-totals border-1 border-[#9b9b9b] h-full w-full max-w-[350px] sm:max-w-1/3">
              <h2 className="p-2 text-sm font-bold border-b border-[#9b9b9b]">
                Cart totals
              </h2>
              <div className="flex flex-col gap-5 p-5">
                ...
                <div className="flex flex-row justify-between border-b border-[#9b9b9b] text-sm text-[#696969]">
                  ...
                  <span>
                    ${" "}
                    {`${
                      checkoutData?.checkout_order.totalDiscount
                        ? `-${checkoutData?.checkout_order.totalDiscount}`
                        : 0
                    }`}
                  </span>
                ...
                <a href="/cart/checkout">
                  <button className="p-5 w-full bg-black text-white hover:bg-[#0573f0] transition-colors duration-500 cursor-pointer">
                    Proceed to checkout
                  </button>
                </a>
              </div>
